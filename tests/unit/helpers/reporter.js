...
async function generateUnitReport(suiteName, testResults, outputFile) {
  const wb = new ExcelJS.Workbook();
  wb.creator = 'TruthGuard Unit Automation';
  wb.created = new Date();

  const COLOURS = { header: 'FF1E3A5F', pass: 'FF00C851', fail: 'FFFF4444', title: 'FF28A745', altRow: 'FFF0F4F8', white: 'FFFFFFFF' };
  ...
  titleCell.value = `TruthGuard API — ${suiteName} | Unit Test Report`;
  ...
  const tcSheet = wb.addWorksheet('⚡ Unit Test Cases', { views: [{ showGridLines: false }] });
  const headers = ['#', 'Test Case ID', 'Test Case Name', 'Target Module', 'Steps', 'Status', 'Duration (ms)', 'Error'];
  ...
  testResults.forEach((t, idx) => {
    const row = tcSheet.addRow([idx + 1, t.id, t.name, t.module || 'API', t.steps || '', t.status, t.duration || 0, t.error || '']);
    row.height = 24;
  ...
  console.log(`✅ Unit Report saved: ${outputFile}`);
}

module.exports = { generateUnitReport, REPORTS_DIR };
